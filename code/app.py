...
def main():
    ...
    if st.session_state.generate_content:
        # Generate the prompt based on the selected learning mode
        if learning_mode == "Lesson":
            prompt = f"As an experienced educator, explain the reasoning behind the key concepts of '{topic}' to a {familiarity} level learner. Structure the explanation to ensure clarity within {st.session_state.time_available} minutes. Contextualize the content with real-world examples and guide the learner through the topic using relatable storytelling. Keep the learner motivated by highlighting practical applications. Finally, evaluate the learning process by recommending 2-3 free, high-quality online courses and relevant YouTube lectures with their links(Dont provide any dummy link that does not exist, provide links that really exist). Also suggest 1-2 projects for hands-on practice. Incorporate {additional_instructions} to further enhance the experience."
        ...
        # Include any uploaded files in the prompt
        if uploaded_files:
            extracted_text = extract_pdf_content(uploaded_files)
            cleaned_text = clean_text(extracted_text)
            prompt += f" Use the following content for reference: {cleaned_text}"

        # Add the language to the prompt if provided
        if preferred_language:
            prompt += f" Generate the content in {preferred_language}."

        # Call the API to generate the content
        response_content = call_api(llm_manager, prompt, llm_info["select_expert"])
